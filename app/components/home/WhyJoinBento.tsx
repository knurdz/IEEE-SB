...
const Segment = ({
  ...
  return (
    <line
      x1={px1 + (px2 - px1) * ratio}
      y1={py1 + (py2 - py1) * ratio}
      x2={px2 - (px2 - px1) * ratio}
      y2={py2 - (py2 - py1) * ratio}
    />
  );
};
...
const HexBentoGraphic = ({
  color,
  strokeColor,
  Shape,
  patternType = "crosses",
  className,
}: HexBentoGraphicProps) => {
  const patternId = `grid-${patternType}-${color.replace("#", "")}`;

  return (
    <div
      className={`absolute pointer-events-none flex items-center justify-center ${className || "bottom-6 left-0 right-0 h-[240px]"}`}
    >
          ...
          >
            <defs>
              <pattern
                ...
                height="80"
                patternTransform={
                  patternType === "diagonal-dashes" ? "rotate(45 400 200)" : ""
                }
              ...
              >
                ...
                {(patternType === "dashes" ||
                  patternType === "diagonal-dashes") && (
                  <path
                    d="M 0 40 L 20 40 M 60 40 L 80 40 M 40 0 L 40 20 M 40 60 L 40 80"
                    fill="none"
                    stroke={color}
                    strokeWidth="3"
                    strokeLinecap="round"
...
export default function WhyJoinBento() {
  return (
    <section className="relative overflow-hidden z-10 py-20 bg-transparent">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {bentoContent.map((item, index) => {
            const { title, description, bg, text, desc, className, Graphic } =
              item;
            ...
            return (
              ...
              >
                <div className="relative z-20 flex flex-col mb-[250px]">
                  <h3
                    className={`text-[26px] font-medium leading-[1.2] tracking-tight mb-3 ${text}`}
                  >
                    {title}
                  </h3>
